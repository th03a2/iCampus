...
export default function MenuCreate({
  ...
  }, [didSubmit, dispatch, setSelectedServices]);
  ...
  const handleSubmit = e => {
    ...
    //   } else {
    //     const _services = [...menu.services],
    //       _menu = { ...form };
    //     _menu.isProfile = isProfile;
    //     _menu.branchId = branchId;
    //     _menu.services = _services.sort();

    //     if (_menu._id) {
    //       dispatch(UPDATE({ data: _menu, id: _menu._id, token }));
    //     } else {
    //       dispatch(SAVE({ form: _menu, token }));
    //     }
    //   }
    // } else {
      ...
      <form className="mt-4" onSubmit={handleSubmit}>
        <MDBRow>
          ...
          <MDBCol size={12} className="mt-2">
            {/* {Services.whereIn(menu.services)?.map((service, index) => ( */}
